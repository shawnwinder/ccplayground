 ...
 */
 ...
 public:
   string predictPartyVictory(string senate) {
     int radiant, dire, nr, nd;
     radiant = dire = nr = nd = 0;
     for (char c : senate) {
       if (c == 'R') nr ++; else nd ++;
     }
     ...
     while (true) {
       for (char& c:senate) {
         if (c == 'R') {
           if (dire > 0) { c = 'X'; nr --; dire --; } else radiant ++;
         } else if (c == 'D') {
           if (radiant > 0) { c = 'X'; nd --; radiant --; } else dire ++;
         }
       }
       if (!nr && nd) return "Dire";
       else if (nr && !nd) return "Radiant";
     }
   }
 };
