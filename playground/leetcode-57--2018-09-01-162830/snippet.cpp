...
public:
    vector<Interval> insert(vector<Interval>& intervals, Interval newInterval) {         
        ...
        auto cmp2 = [] (const Interval& a, const int& b) {
            return a.end < b;
        };
        sort(intervals.begin(), intervals.end(), cmp);
        auto it = lower_bound(intervals.begin(), intervals.end(), newInterval.start, cmp2);
        if (it == intervals.end() || it->start > newInterval.end) intervals.insert(it, newInterval);
        else {
            int l = min(it->start, newInterval.start);
            int r = max(it->end, newInterval.end);
            while (it != intervals.end() && it->end < r) it = intervals.erase(it);
            if (it != intervals.end() && it->start <= r) r = it->end, it = intervals.erase(it);
