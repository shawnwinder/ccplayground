...
public:
    int strongPasswordChecker(string s) {
        ...
        //  (1) Try to delete one letter from repetitions with (length % 3) == 0. Each deletion
        // decreases replacement by 1 (2) Try to delete two letters from repetitions with (length %
        // 3) == 1. Each deletion decreases repalcement by 1 (3) Try to delete multiple of three
        // letters from repetions with (length % 3) == 2. Each deletion (of three letters) decreases
        // repalcements by 1
        for (int i = 0, numLetters, dec; i < 3; i++) {
            for (auto it = lenCnts[i].begin(); it != lenCnts[i].end(); it++) {
                if (i < 2) {
                    ...
                    toDelete
                        += dec * numLetters; // dec is the number of repetitions we'll delete from
